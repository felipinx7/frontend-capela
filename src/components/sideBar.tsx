            ...
            <div className="w-full flex flex-col items-center justify-between gap-4">
                {sectionsLinks.map((link) => (
                    <button key={link.section} onClick={() => handleSection(link.section)} className={`${link.section === nameSection ? "bg-white text-primary-100 font-semibold" : "bg-transparent text-white font-regular "} group hover:bg-white duration-500 ease-in-out transition-all hover:text-primary-100 hover:font-bold flex cursor-pointer w-full items-center p-3 rounded-2xl justify-start gap-3`}>
                        ...
                        <p className="">{link.nameSection}</p>
                    </button>
                ))}
            </div>

            {/* buttom logout  */}
            <button onClick={() => functionLogout()} className="hover:bg-gray-300 cursor-pointer w-[90%] rounded-[0.9rem] p-3 bg-white text-primary-100 font-bold flex items-center justify-center gap-2">
                <IconLogout className="w-5" /> LOGOUT
            </button>
        </article>
    )
}